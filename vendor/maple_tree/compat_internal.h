...
static inline void *kmem_cache_alloc(struct kmem_cache *s, gfp_t flags) {
	...
	if (posix_memalign(&ptr, sizeof(struct maple_node), sizeof(struct maple_node)) != 0)
		return NULL;
	memset(ptr, 0, sizeof(struct maple_node));
	return ptr;
}
static inline void kmem_cache_free(struct kmem_cache *s, void *ptr) {
	free(ptr);
}
...
static inline int kmem_cache_alloc_bulk(struct kmem_cache *s, gfp_t flags, size_t size, void **ptr) {
	size_t i;
	for (i = 0; i < size; i++) {
		if (posix_memalign(&ptr[i], sizeof(struct maple_node), sizeof(struct maple_node)) != 0)
			goto error;
		memset(ptr[i], 0, sizeof(struct maple_node));
	}
	return size;
error:
